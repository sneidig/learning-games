...
interface BaseLevel {
  ...
  /**
   * Why the correct answer is correct — shown under the success indicator once
   * the level is solved, so getting it right also means understanding it.
   */
  explain?: string
  /**
   * An optional nudge the player can reveal BEFORE answering, so they can reason
   * it out instead of guessing. Points at the reasoning; never states the answer.
   */
  hint?: string
}
