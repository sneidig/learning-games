...
export function LevelPlayer({
  ...
  // whether the player revealed the before-answer hint
  ...
  return (
    <section className="player">
      ...
      {level.hint &&
        !solved &&
        (hintShown ? (
          <p className="player__hint">💡 {level.hint}</p>
        ) : (
          <button className="link player__hintbtn" onClick={() => setHintShown(true)}>
            💡 Need a hint?
          </button>
        ))}
      ...
      {solved && (
        <div className="result result--pass" role="status">
          ...
          <div>
            <strong className="result__title">✅ Correct — here's why</strong>
            <p className="result__explain">{level.explain ?? 'Nice — on to the next one.'}</p>
          </div>
        </div>
      )}
